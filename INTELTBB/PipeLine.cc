...
MyInputFilter::MyInputFilter( FILE* input_file_ ) : filter(serial_in_order), input_file(input_file_), next_slice( TextSlice::allocate(MAX_CHAR_PER_INPUT_SLICE)) { }
...
MyOutputFilter::MyOutputFilter( FILE* output_file ) : tbb::filter(serial_in_order), my_output_file(output_file) { }
...
int run_pipeline( int nthreads ) {
    ...
    cout << "Time Spend :: " << (t1-t0).seconds() << endl;
    return 1;
}
...
int main( int argc, char* argv[] ) {
	...
	bool serialrunning = true;
        ...
        if (serialrunning){ // serial run
            cout << "serial run   " << endl;
            tbb::task_scheduler_init init_serial(1);
            if (!run_pipeline(1))
                return 1;
        } else { // parallel run (number of threads is selected automatically)
